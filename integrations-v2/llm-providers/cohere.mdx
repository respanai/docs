---
title: "Cohere"
description: "Trace Cohere SDK calls with Respan — auto-instrumented spans, gateway routing, and full observability."
---
...
The [Cohere SDK](https://github.com/cohere-ai/cohere-python) is the official Python client for Cohere's language models, supporting chat, embed, and rerank. Respan can auto-instrument all Cohere calls for tracing, route them through the Respan gateway, or both.
...
<Tab title="Tracing">
```bash
pip install respan-ai opentelemetry-instrumentation-cohere cohere python-dotenv
```
</Tab>
<Tab title="Gateway">
```bash
...
<Tab title="Both">
```bash
pip install respan-ai opentelemetry-instrumentation-cohere cohere openai python-dotenv
```
</Tab>
</Tabs>
...
# Calls go directly to Cohere, auto-traced by Respan
...
```python
# Cohere
response = client.chat.completions.create(model="command-r-plus", messages=messages)
